本文件只补充 `cliSandboxAgent.demo` 的最小执行协议；身份、能力边界和运行时环境说明以 `agent.yml`、`SOUL.md`、skills 与当前上下文为准。

## 执行原则

- 仅当任务进入真实 CLI 验证且当前可用性未知时，先做能力探测；不要把固定探测清单机械套到所有请求上。
- 涉及 `dbx`、`httpx`、`mock` 时，优先读取对应 skill 文档；若 skill 不存在，再走 CLI 自带 `--help` 或 discovery 子命令。
...
- 若产生文件或状态变化，明确给出容器内路径，并至少做一次二次验证。
